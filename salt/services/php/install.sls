...
php_redis_source:
  file.managed:
    ...
    - source: salt://services/php/files/phpredis-2.2.4.tar.gz
    - user: root
    - group: root
    - mode: 644
  cmd.run:
    - cwd: /usr/local/src
    - name: tar -xzf phpredis-2.2.4.tar.gz
...
php_redis_compile:
  cmd.run:
    - cwd: /usr/local/src/phpredis-2.2.4
    - name: /usr/local/php/bin/phpize && ./configure --with-php-config=/usr/local/php/bin/php-config && make && make install
    - requires:
