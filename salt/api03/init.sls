...
/usr/local/tomcat-{{ project }}/conf/server.xml:
  file.managed:
    - source: salt://api03/files/server-{{ project }}.xml
    - user: root
    - mode: 644

/usr/local/tomcat-{{ project }}/bin/setenv.sh:
  file.managed:
    - source: salt://api03/files/setenv-{{ project }}.sh
    - user: root
    - mode: 755
    - template: jinja
...
tomcat-{{ project }}_service:
  file.managed:
    ...
    - source: salt://api03/files/tomcat-{{ project }}.service
    - user: root
    - mode: 755
    - template: jinja
    - defaults:
...
/etc/logstash/conf.d/shipper-api03.conf:
  file.managed:
    - source: salt://api03/files/shipper-api03.conf
    - user: root
    - group: root
    - mode: 644
    - template: jinja
